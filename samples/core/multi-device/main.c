...
int main(int argc, char* argv[])
{
    ...
    cl_uint subdev_count = 0, max_compute_units = 0;
    ...
    OCLERROR_RET(clCreateSubDevices(dev, subdevices_properties,
                                    max_compute_units, NULL, &subdev_count),
                 error, dev);
    ...
    const size_t input_size = sizeof(cl_float) * pad_x_dim * pad_y_dim;
    const size_t output_size = sizeof(cl_float) * x_dim * y_dim;
    const size_t mask_size = sizeof(cl_float) * mask_dim * mask_dim;
    ...
    // out-of-bounds.
    MEM_CHECK(h_input_grid = (cl_float*)malloc(input_size), error, prg);
    if (diag_opts.verbose)
    {
    ...
    MEM_CHECK(h_output_grid = (cl_float*)malloc(output_size), error, hinput);

    // Initialize convolution mask.
    MEM_CHECK(h_mask = (cl_float*)malloc(mask_size), error, houtput);
    if (diag_opts.verbose)
    {
    ...
    OCLERROR_PAR(dev_input_grid = clCreateBuffer(
                     context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
                     input_size, h_input_grid, &error),
                 error, hmask);
    OCLERROR_PAR(dev_output_grid = clCreateBuffer(
                     context, CL_MEM_WRITE_ONLY | CL_MEM_HOST_READ_ONLY,
                     output_size, NULL, &error),
                 error, bufin);
    OCLERROR_PAR(dev_mask = clCreateBuffer(
                     context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, mask_size,
                     h_mask, &error),
                 error, bufout);
    ...
    for (; subdevice < subdev_count; ++subdevice)
    {
        ...
        if (diag_opts.verbose)
        {
            printf("\n  Creating kernel and command queue of sub-device %ld...", subdevice);
            fflush(stdout);
        }
        ...
        OCLERROR_PAR(
            sub_queues[subdevice] = clCreateCommandQueueWithProperties(
        ...
        OCLERROR_PAR(sub_output_grids[subdevice] = clCreateSubBuffer(
                     ...
                     error, subbufin);
        OCLERROR_PAR(sub_masks[subdevice] = clCreateSubBuffer(
                         dev_mask, CL_MEM_READ_ONLY,
                         CL_BUFFER_CREATE_TYPE_REGION, &mask_region, &error),
                     error, subbufout);
        ...
        OCLERROR_RET(clSetKernelArg(convolutions[subdevice], 0,
                                    sizeof(cl_mem),
                                    &sub_input_grids[subdevice]),
                     error, subbufmask);
        OCLERROR_RET(clSetKernelArg(convolutions[subdevice], 1,
                                    sizeof(cl_mem),
                                    &sub_output_grids[subdevice]),
                     error, subbufmask);
        OCLERROR_RET(clSetKernelArg(convolutions[subdevice], 2,
                                    sizeof(cl_mem),
                                    &sub_masks[subdevice]),
                     error, subbufmask);
        OCLERROR_RET(clSetKernelArg(convolutions[subdevice], 3,
                                    sizeof(size_t), &x_dim),
                     error, subbufmask);
        OCLERROR_RET(clSetKernelArg(convolutions[subdevice], 4,
                                    sizeof(size_t), &grid_midpoint),
                     error, subbufmask);
    }
    ...
    const size_t mid_output_size = sizeof(cl_float) * mid_output_count;
    MEM_CHECK(concatenated_results = (cl_float*)malloc(output_size), error,
              event2);
    OCLERROR_RET(clEnqueueReadBuffer(sub_queues[0], sub_output_grids[0],
                                     CL_BLOCKING, 0, mid_output_size,
                                     concatenated_results, 0, NULL, NULL),
                 error, result);
    OCLERROR_RET(clEnqueueReadBuffer(sub_queues[1], sub_output_grids[1],
                                     CL_BLOCKING, mid_output_size, mid_output_size,
                                     concatenated_results + mid_output_count, 0,
                                     NULL, NULL),
                 error, result);
...
event1:
    OCLERROR_RET(clReleaseEvent(events[0]), end_error, subbufmask);
subbufmask:
    if(subdevice == 1)
    {
        OCLERROR_RET(clReleaseMemObject(sub_masks[1]), end_error, subbufmask0);
    }
subbufmask0:
    OCLERROR_RET(clReleaseMemObject(sub_masks[0]), end_error, subbufout);
subbufout:
    if(subdevice == 1)
    {
        OCLERROR_RET(clReleaseMemObject(sub_output_grids[1]), end_error, subbufout0);
    }
subbufout0:
...
subbufin:
    if(subdevice == 1)
    {
        OCLERROR_RET(clReleaseMemObject(sub_input_grids[1]), end_error, subbufin0);
    }
subbufin0:
...
subqueue:
    if(subdevice == 1)
    {
        OCLERROR_RET(clReleaseCommandQueue(sub_queues[1]), end_error, subqueue0);
    }
subqueue0:
